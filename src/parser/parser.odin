...
parse_operand :: proc(p: ^Parser, lhs: bool) -> ^ast.Expr {
	...
	case .Tensor:
		tok := expect_token(p, .Tensor)
		expect_token(p, .Open_Bracket)
		
		// Parse shape dimensions - allow _ for runtime-determined dimensions
		shape: [dynamic]^ast.Expr
		for {
			if p.curr_tok.kind == .Ident && p.curr_tok.text == "_" {
				// Runtime-determined dimension
				underscore := advance_token(p)
				dim := ast.new(ast.Ident, underscore.pos, end_pos(underscore))
				dim.name = "_"
				append(&shape, dim)
			} else {
				// Compile-time known dimension
				dim := parse_expr(p, false)
				append(&shape, dim)
			}
		...
		tt := ast.new(ast.Tensor_Type, tok.pos, elem)
		tt.tok_pos = tok.pos
		tt.shape = shape
		tt.elem = elem
		return tt
